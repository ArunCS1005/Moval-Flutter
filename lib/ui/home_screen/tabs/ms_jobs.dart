...
import '../../util_ui/message.dart';
import '../widgets/jobs_item.dart';

class MSJobs extends StatefulWidget {
  ...
  static State<MSJobs>? of(BuildContext context) =>
      context.findRootAncestorStateOfType<_State>();
}

class _State extends State<MSJobs> with AutomaticKeepAliveClientMixin {
  ...
  Widget build(BuildContext context) {
    super.build(context);
    return RefreshIndicator(child: _body(), onRefresh: _refresh);
  }
  ...
  Future _getLocalData() async {
    ...
    final response = await LocalJobs.getJobs(
        platformType: platformTypeMS, jobType: widget.jobType);
    final offlineJobs = widget.jobType == pending
        ? await LocalJobs.getJobs(
            platformType: platformTypeMS, jobType: offline)
        : [];
  ...
  Future _getData() async {
    ...
    if (response == Api.defaultError) {
      ...
      _dataList.addAll(response['values']);
      _currentPage = response['pagination']['current_page'];
      LocalJobs.saveJobs(
          platformType: platformTypeMS,
